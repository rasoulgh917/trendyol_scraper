...
def import_product(tablename, product_dict):
    ...
    product.product_images = str(product_dict['product_images']).encode()
    ...
    product.delivery_info = str(product_dict['delivery_info']).encode()
    ...
    try:
        product.product_seller_score = str(product_dict['product_seller']['sellerScore']).encode()
    except:
